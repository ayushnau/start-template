...
import CurrencyPairBadge from "../../portfolio/SortFilters/CurrencyPairBadge";
import FilterBadge from "../../portfolio/SortFilters/FilterBadge";
import { useBetaForm } from "@locoworks/reusejs-toolkit-react-hooks";
import { TradeKeyList, removeTrailingCommas } from "utils";
import FilterBadgePair from "../../portfolio/SortFilters/FilterBadgePair";
...
const FilterUnifiedModal = React.forwardRef(
	(props: ModalInterface, ref: any) => {
		const { filterKeysList, filterFormValues, type } = props;
		const [empty, setEmpty] = useState<any>(false);

		const filterFormObject: any = {};
		const emptyObject: any = {};
		filterKeysList.map((value: any) => {
			const currentFilterKey = value.formKey;
			emptyObject[currentFilterKey] = "";
			filterFormObject[currentFilterKey] = filterFormValues[currentFilterKey];
		});
		...
		const addOrRemoveBankName = (bankName: string) => {
			if (form.value.bank_name.split(",").includes(bankName)) {
				form.setField(
					"bank_name",
					removeTrailingCommas(
				...
				form.setField(
					"bank_name",
					removeTrailingCommas(
						form.value.bank_name.split(",").concat(bankName).join(","),
					),
				);
			}
		};
				...
				case "maturity_months":
					return value.map((date: any, index: number) => {
						...
						return (
							...
							>{`${moment(currentDate)
								.startOf("month")
								.format("MMM")} '${moment(currentDate).format(
								"YY",
							)}   (${count})`}</FilterBadge>
						);
					});
					...
					<div className="flex justify-between items-center px-5 py-2 border-b w-full sticky top-0 z-[999] bg-white pt-[10px]">
						...
						<label
							...
							onClick={() => {
								setEmpty(true);
								filterKeysList.map((value: any) => {
									const formKey = value.formKey;
									form.setField(formKey, "");
								});
							}}
						>
							Clear
