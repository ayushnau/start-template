...
import AddTradesBulkUpload from "./AddTradesBulkUpload";
import Validate from "./Validate";
import Loading from "./Loading";
import Success from "./Success";
import { validateBulkUploadTrades } from "services";
import { BackArrowIcon, DownloadFormat } from "icons";
import showTradeListModal from "./showTradeListModal";
import { convertCsvToJson, convertCsvFileToCsv } from "services";
...
const Modal = React.forwardRef((props: any, ref: any) => {
	...
	const [recordSize, setRecordSize] = useState(props.tradesNumber);
	const [data, setData] = useState<any>([]);
	const [errors, setErrors] = useState<any>({});
	const [tradeListShown, setTradeListShown] = useState(false);
	...
	const handleCallValidate = async () => {
		if (file !== null) {
			...
			try {
				...
				const payload = {
					trades_list: jsonData,
					ledger_id: props.ledgerId,
				};
				const response: any = await validateBulkUploadTrades(payload);
				if (
	...
	return (
		...
		>
			<>
				<div className="flex items-center justify-between py-5 px-6 border-b border-mine-shaft-2">
					...
					<div className="flex items-center gap-x-2">
						...
						<a
							target="_blank"
							onClick={() => {
								fetch("/bulk_upload_trades_format.csv")
									.then((resp) => {
										return resp.blob();
									})
									.then((blob) => {
										...
										a.href = url;
										a.download = "bulk_upload_trades_format.csv";
				...
				{validateErrorComponent || loadingErrorComponent ? (
					<div className="px-6 pt-2 mt-2 -mb-2">
						{validateErrorComponent ? (
							<div className="px-[6px] py-[5px] bg-sunset-orange-1 text-sunset-orange-3 rounded-md">
								...
								<p>
									Please download the right format provided, ensure all the
									fields are filled as per the requirement, and try again.
								</p>
							</div>
						) : (
							<></>
				...
				{isLoading || isValidating || isSuccess ? (
					<>
						<div
							className={
								"px-6 pt-4 mt-2 pb-4 flex flex-col flex-1 h-full text-center w-[505px] "
							}
						>
							<div className="p-8 h-full flex flex-col items-center justify-center gap-y-6 max-h-[242px]">
								...
								{isValidating ? (
									<Validate type="trades" recordSize={recordSize} />
								) : (
									<></>
								)}
								{isLoading ? (
...
const showBulkUploadModal = async ({
	callbackYes,
	callbackNo,
	showSuccess = false,
	tradesNumber = 0,
...
export default showBulkUploadModal;
