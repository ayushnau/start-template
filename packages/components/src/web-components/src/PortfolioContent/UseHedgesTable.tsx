...
export interface UseHedgesTableInterface {
	dataSet?: any;
}

const UseHedgesTable: React.FC<UseHedgesTableInterface> = ({ dataSet }) => {
	const tableRef = React.useRef<any>();
	const { eefcId } = useParams();

	const eefc = useSelector((state: any) => {
		if (eefcId) {
			return state?.portfolioHedgesList?.hedgeList[eefcId];
		}
	});
	...
	const validateAmount = () => {
		if (+form.value.amount > +eefc.remaining_amount) {
			return false;
		}
		return true;
	};
