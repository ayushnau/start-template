...
export interface UnLinkedHedgesTableInterface {
	dataSet?: any;
}

const UnLinkedHedgesTable: React.FC<UnLinkedHedgesTableInterface> = ({
	dataSet,
}) => {
	...
	const validateAmount = () => {
		if (+form.value.amount > +trade.unhedged_amount) {
			return false;
		}
		return true;
	};
			...
			case "mtm":
				...
				return (
					<TagRenderer
						...
						tableSrc={"hedge"}
						currentMarketRates={hedge_data.current_market_rates}
			...
			case "amount_to_be_used":
				return (
					<AmountInput
						...
						errorStates={[
							"Amount Cant be Greater than Remaining unlinked or Available amount",
						]}
					/>
				);
			default:
					...
					{HedgeUtilisationHeaderColumnList.map((ele, index) => {
						return (
							...
							>
								...
								{ele === "Gain/Risk" && (
									<span className="cursor-pointer text-color-black-5 text-sm justify-center">
										{" ⓘ"}
									</span>
								)}
...
export default UnLinkedHedgesTable;
