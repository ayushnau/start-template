...
import { setbcCostCalculatorRepaymentRate } from "store/src/bcCostCalculatorSlice";

export interface BcCostCalculatorHomeInterface {
	web?: boolean;
}
const BcLoanRepayment: React.FC<BcCostCalculatorHomeInterface> = ({
	web = false,
}) => {
	const { repayment_rate } = useSelector((state: StoreState) => {
	...
	const form = useBetaForm({
		loan_repay_rate: repayment_rate,
	});
	React.useEffect(() => {
	...
	const handleContinueClick = async () => {
		await dispatch(
			setbcCostCalculatorRepaymentRate(form.value.loan_repay_rate),
		);
		navigate(
			web === true
			...
			<div className="w-full h-full flex flex-col justify-between">
				<BorrowingCostWrapper>
					<BorrowingCostTextContent
						...
						description="Please provide the rate at which the BC/SC/LAI loan was repaid to the bank:"
					/>
				</BorrowingCostWrapper>
...
export default BcLoanRepayment;
