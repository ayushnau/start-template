...
import { setfcnrCostCalculatorSpread } from "store/src/fcnrCostCalculatorSlice";
import showInfoModal from "../../Modals/InfoModal";
import moment from "moment";

export interface FcnrSpreadInterface {
	web?: boolean;
}

const FcnrSpread: React.FC<FcnrSpreadInterface> = ({ web = false }) => {
	const navigate = useNavigate();
	const dispatch = useDispatch();
	const { spread ,loan_due_date} = useSelector(
		(state: StoreState) => state.fcnrCostCalculatorSlice,
	);
	...
	const handleContinueClick = () => {
		dispatch(setfcnrCostCalculatorSpread(form.value.float_spread));
			...
			<div className="w-full h-full flex flex-col justify-between ">
				<BorrowingCostWrapper>
					<BorrowingCostTextContent
						...
						description="Please enter the spread on the floating benchmark, accounting for any upfront adjustments made:"
					/>
				</BorrowingCostWrapper>
