...
import HedgeDetails from "../../.././../../../apps/wiredup/src/pages/fx/portfolio/Hedges/HedgeDetails";
...
import { formatDate, formatNumberWithCommas, checkDate } from "utils";
import { useModalNavigation } from "services";
//TODO: Change this image to confetti amimation
// import Image1 from "../../.././../../../apps/wiredup/src/icons/backgroundSuccess.png";

const INFODETAILS = [
	{
		title: "Balance amount",
		description: [
			`Remaining amount in a hedge refers to the hedge position that is available to be used for managing potential risks due to changes in market conditions, such as fluctuations in exchange rates.`,
		],
	},
	{
		title: "Linked amount",
	...
	},
	{
		...
		description: [
			`Unlinked amount is the remaining portion of the hedged position that has not been assigned to any specific trade(s) added on WiredUp. This portion might be used for future trades for managing general risk exposure or for other strategic purposes.`,
		],
	},
	{
		title: "Hedge rate",
...
const Hedge = () => {
	...
	const cancelHedge = async () => {
		...
		if (unlinkedAmount == "0") {
			...
			await CancelUnlinkedHedgeModal({
				...
				web: true,
				callbackYes: () => {
					switchModalScreen(`cancel-hedge/${hedgeId}`);
				},
			});
		}
	};
	...
	const useHedge = async () => {
		...
		if (unlinkedAmount == "0") {
			...
			await CancelUnlinkedHedgeModal({
				...
				web: true,
				callbackYes: () => {
					switchModalScreen(`use-hedge/${hedgeId}`);
				},
			});
		}
	};
			...
			<div className="text-bean-red-dark text-base font-bold leading-6 -tracking-[.3px]">
				...
				<div className="flex items-center justify-between gap-x-4 mt-2">
					<PrimaryButton
						...
						onClick={() => {
							switchModalScreen(`update-unlinked-hedge/${hedgeId}`);
						}}
					...
					<SecondaryButton
						...
						onClick={() => {
							switchModalScreen(`update-hedge/${hedgeId}`);
						}}
	...
	return (
		<Loader
			// loadingClasses="w-[525px] mx-0 inset-[unset] opacity-50"
			...
			successComponent={
				<div className="bg-white h-full flex flex-col">
					<Header
						...
						showEditText={true}
						editAction={() => {
							switchModalScreen(`update-hedge/${details?.uuid}`);
						}}
